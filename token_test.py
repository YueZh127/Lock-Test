...
class TokenContract(object):
    ...
    def approve_token(self, private_key, address, allowance):
        ...
        logger.info(f'approve token for {account.address}.. ')
        ...
        # logger.error("approve_tx: ", approve_tx)
        ...
        logger.info(f"signed_approve_tx_id: {self.Web3.toHex(signed_tx_id)}")
    ...
    def get_balance(self, owner_private_key, block_identifier=None):
        account = self.Web3.eth.account.privateKeyToAccount(owner_private_key)
        if block_identifier is not None:
            return self.token_contract.functions.balanceOf(account.address).call(block_identifier=block_identifier)
